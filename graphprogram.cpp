...
const int vertices = 10000;	//change this value to create larger graphs
...
char graph_menus_functions(void)
{
	...
	cout << "\t\tZ for maximum independent set with heuristics" << endl;
	...
	}while ((operation != 'S')&& (operation != 'D') && (operation != 'M') && (operation  != 'T') && (operation != 'K') && (operation != 'P') && (operation != 'B')&& (operation != 'A'  )&& (operation!='J')&&(operation != 'N')&& (operation != 'X')&&(operation != 'Z'));
	return operation;
}
//------------------------end graph menus
//------------------------begin file input 
...
//this function is used for pivoting in MIS and clique calculation
int deltaX(int vertex, int Qplus[vertices][vertices], short k) {
	int dX = 0;
	for (int x = 0; x < nvertices; x++) {
		if (adj_cost_matrix[vertex][x] == 1 && Qplus[k][x] != 0) {
			dX += 1;
		}
	}
	return dX;
...
//this function is used by MIS and clique calculation
short step2plus(short k, char eq38, char& neweq38, char print_MIS, char directed, char MIS_or_clique, int Qplus[vertices][vertices], int Qminus[vertices][vertices], int S[vertices][vertices], int X[vertices], int newQplus[vertices][vertices], int newQminus[vertices][vertices], int newS[vertices][vertices], int newX[vertices], int neighbors[vertices][vertices])
{
	short a, xk = nvertices;
	int pivot = 0;
	//find the lowest deltaX in Qminus using equation 3.9
	int minDX = 10000000;
	int xStar = 0;
	for (a = 0; a < nvertices; a++) {
		if (Qminus[k][a] != 0) {
			int min = deltaX(Qminus[k][a], Qplus, k);
			if (min < minDX) {
				minDX = min;
				xStar = Qminus[k][a];
			}
	...
	if (xStar != 0) {
		//chose the new xk using xstar as described on page 35
		for (int x = 0; x < nvertices; x++) {
			if (adj_cost_matrix[xStar][x] != 0 && Qplus[k][x] != 0) {
				pivot = Qplus[k][x];
			}
		}
	}
	...
	for (a = 0; a < nvertices; a++)
	{
		...
		else if ((Qplus[k][a] != 0) && (Qplus[k][a] < xk))
		{
			xk = Qplus[k][a];//find the lowest numbered vertex in Qplus
		}
		Qplus[k + 1][a] = Qplus[k][a];//set Q+ (k+1) to Q+ k, same for Q- and S
		Qminus[k + 1][a] = Qminus[k][a];
		S[k + 1][a] = S[k][a];
		neighbors[k + 1][a] = neighbors[k][a];
	}

	//find all neighbors of xk
	for (a = 0; a < nvertices; a++) {
		if (adj_cost_matrix[xk - 1][a] != 0) {
			neighbors[k + 1][a] = a + 1;
		}
	}
	neighbors[k + 1][xk - 1] = xk;

	X[k] = S[k + 1][xk - 1] = xk;//add xk to the set X of vertices and to S

	//find the min degree vertex but exluding xk and its neighbors
	int minDegree = 10000000;
	for (a = 0; a < nvertices; a++)
	{
		int degree = 0;
		if (neighbors[k + 1][a] == 0) {
			for (int i = 0; i < nvertices; i++) {
				if (neighbors[k + 1][i] == 0) {
					degree += adj_cost_matrix[a][i];
				}
			}
			if (degree < minDegree) {
				minDegree = degree;
	...
	//set Q+0 to vertices with smallest degree
	for (a = 0; a < nvertices; a++)
	{
		int degree = 0;
		if (neighbors[k + 1][a] == 0) {
...
short step4plus(short k, char eq38, char& neweq38, char print_MIS, short max_ind, short& newmax_ind, char done, char& newdone, char directed, char MIS_or_clique, int Qplus[vertices][vertices], int Qminus[vertices][vertices], int S[vertices][vertices], int X[vertices], int newQplus[vertices][vertices], int newQminus[vertices][vertices], int newS[vertices][vertices], int newX[vertices], int neighbors[vertices][vertices])
{
	...
	if (Qplus_size != 0)	//i.e. Q+ is not empty
	{
		...
		k = step2plus(k, eq38, eq38, print_MIS, directed, MIS_or_clique, Qplus, Qminus, S, X, Qplus, Qminus, S, X, neighbors);
	}
	else if (Qminus_size == 0)//Q+ and Q- are empty
	{
		...
		k = step5plus(k, eq38, eq38, print_MIS, done, done, MIS_or_clique, Qplus, Qminus, S, X, Qplus, Qminus, S, X, neighbors);
	}
...
//MIS with heuristics
int MISplus(char directed, bool user, char print_MIS)
{
	int Qminus[vertices][vertices] = { 0 }, Qplus[vertices][vertices] = { 0 }, S[vertices][vertices] = { 0 }, X[vertices] = { 0 };//Q-, Q+, S, and X for MIS calculation
	int neighbors[vertices][vertices] = { 0 }; //for keeping track of which nodes have been removed for the vertex ordering
	short a, k = 0, max_ind = 0;//set state of k and max_ind(the maximum size of an MIS)
	double start, finish, duration;	//will be used for determining time of calculation
	char eq38 = '?';		//says whether or not equation 3.8 is satisfied
	char done = 'N';
	...
	//find the vertex with the smallest degree for vertex ordering as per https://en.wikipedia.org/wiki/Bron%E2%80%93Kerbosch_algorithm
	int minDegree = 10000000;
	for (a = 0; a < nvertices; a++)
	{	
		int degree = 0;
	...
	for (a = 0; a < nvertices; a++)
	{
		...
		if (degree == minDegree) {
			Qplus[0][a] = (a + 1);//set Q+0 to vertices with smallest degree
		}
	}
	...
	k = step2plus(k, eq38, eq38, print_MIS, directed, 'm', Qplus, Qminus, S, X, Qplus, Qminus, S, X, neighbors);
	do {
		if (eq38 == 'S')	//equation 3.8 is satisfied, go to step 5
		{
			k = step5plus(k, eq38, eq38, print_MIS, done, done, 'm', Qplus, Qminus, S, X, Qplus, Qminus, S, X, neighbors);
		}
		else		//equation 3.8 is not satisfied, go to step 4
		{
			k = step4plus(k, eq38, eq38, print_MIS, max_ind, max_ind, done, done, directed, 'm', Qplus, Qminus, S, X, Qplus, Qminus, S, X, neighbors);
		}
	} while (done != 'Y');
	finish = clock();
	duration = (finish - start) / CLOCKS_PER_SEC;		//calculate amount of time it took
...
do{
	...
	do{
		...
		if ('Z' == function)
		{
			MISplus(directed, user, print_MIS);
		}
